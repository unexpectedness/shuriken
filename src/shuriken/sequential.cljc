(ns shuriken.sequential
  ...
  (:require [clojure.spec.alpha :as    s]
   ...
   #?(:clj  [ubergraph.core     :as    u])
   #?(:clj  [ubergraph.alg      :as    alg])
            [shuriken.namespace :refer [import-vars]]
            [shuriken.spec      :refer [conform!]]
  #?(:cljs  [cljs.analyzer.api  :as    ana]))
  #?(:cljs (:require-macros [shuriken.sequential :refer [def-compound-seq-fns]])))

#?(:clj  (import-vars clojure.core/reduce1)
   :cljs (defn reduce1
           ([f coll]
            (let [s (seq coll)]
              (if s
     ...
     (if (associative? coll)
       ...
       (do (when-not (<= n (count coll))
             (throw #?(:clj  (IndexOutOfBoundsException.)
                       :cljs (js/Error. "Index out of bounds"))))
           (concat (take n coll)
                   [v]
                   (drop (inc n) coll)))))))
...
(def ^:private seq-suffixes-2 '{s   set
                                ...
                                m   (into {})
                                str (apply str)})
(def ^:private seq-blacklist  '#{takes})
...
(defmacro def-compound-seq-fns []
  `(do ~@(for [seq-fn           seq-fns
               ...
               :let [resolve    #?(:clj resolve :cljs ana/resolve)
                     seq-macro? (-> seq-fn resolve meta :macro)
